...
#WE WILL USE THE SAVE FUNCTION IN RSTUDIO
...
graph + geom_point() + labs(title = "geom_point()")  #NOTE THE CHANGE of OPTS to LABS
#saveInImageDirectory("04 Tutorial Point.png")  #We do not need to save

#CHANGE THE SHAPP OF THE POINTS AND GRAPH TITLE
graph + geom_point(shape = 17) + labs(title = "geom_point(shape = 17)") #change opts to labs

#saveInImageDirectory("04 Tutorial Triangle.png")

#CHANGE THE POINT SIZE AND THE TITLE
graph + geom_point(size = 6) + labs(title = "geom_point(size = 6)")  #change opts to labs
...
festivalHistogram + geom_histogram(binwidth = 0.4) + #ADDJUST THE BIN WIDTH
...
festivalDensity <- ggplot(festivalData2, aes(day1)) #CREATE THE GRAPH OBJECT
festivalDensity + 
  geom_density() + # USE THE SMOTHED HISTOGRAM
  labs(x = "Hygiene (Day 1 of Festival)", y = "Density Estimate") #ADD LABELS
