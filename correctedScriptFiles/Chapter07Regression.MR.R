...
#assess assumptions
#normality
qplot.mortality<- qqnorm(pubs$mortality,
                         xlab = "Theoretical Quantiles, Mortality", 
                         ylab = "Sample Quantiles, Mortality")
qqline(pubs$mortality)

qplot.bars<- qqnorm(pubs$pubs,
                xlab = "Theoretical Quantiles, Pubs", 
                ylab = "Sample Quantiles, Pubs")
qqline(pubs$pubs)


#assess assumption of linearity
scatter <- ggplot(pubs, aes(pubs, mortality)) +  #CREATE THE GRAPH OBJECT
  geom_point() + #ADD THE DATAPOINTS
  labs(x = "Pubs", y = "Mortality") #ADD NICE LABELS  #DOESN'T USE THE OPTS COMMAND ANYMORE
scatter  #view the scatterplot



#Run a simple linear gregression with mortality as the DV and pubs as the IV
pubReg <- lm(mortality ~ pubs, data = pubs)
summary(pubReg)  #get the summary information (slope, F values)
# Walk through the output

#plot our regression
scatter <- ggplot(pubs, aes(pubs, mortality)) +  #CREATE THE GRAPH OBJECT
  geom_point() + #ADD THE DATAPOINTS
  geom_smooth(method = "lm", colour = "Blue", se = F) + #ADD BEST FIT LINE (LINEAR MODEL), SET THE COLOUR, NO STANDARD ERROR
  labs(x = "Pubs", y = "Mortality") #ADD NICE LABELS  #DOESN'T USE THE OPTS COMMAND ANYMORE
scatter


#assess how good our fit is
#look for outliers cases
resid(pubReg)  #get the residuals (error) for each data point
rstandard(pubReg) #standardized residulas (zscores)

# we can add the residuals to our dataframe 
pubs$residuals <- resid(pubReg)  
View(pubs)

# Now Do it for the standardized residuals
pubs$rstandard <- rstandard(pubReg) #standardized residulas (zscores)
View(pubs)

#-----open the examData file that we have been using and 
#-----run a simple linear regression with Exam as the outcome variable
#-----and Anxiety as the predictor
...
# we can add the residuals to our dataframe 
# modify this code so that it works
pubs$residuals <- resid(pubReg)  
View(pubs)
...
rstudent(pubReg) #studentized (tscores) residuals 
round(cooks.distance(pubReg),3)  #cooks distance
dfbeta(pubReg)
dffits(pubReg)
hatvalues(pubReg)  #leverage
covratio(pubReg)

PearsonResidual <- (resid(pubReg)-mean(resid(pubReg)))/sd(resid(pubReg))
View(PearsonResidual)
