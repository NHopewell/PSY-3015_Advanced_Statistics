...
x <-c(8,6,4,3,7,6,2,5,2,8)
y <-c(1,1,0,1,1,1,0,1,0,1)

classData <-data.frame(x,y)
View(classData)

#run logistic regression
classModel<-glm(y~x, family=binomial(), data=classData)
summary(classModel)

anova(classModel,test="Chisq")

#get odds ratio:
exp(coef(classModel))

exp(confint(classModel))

round(exp(cbind(oddsratio = coef(classModel),confint(classModel))),3)

#get predicted values and residuals
classData$predicted <- round(fitted(classModel),3)
classData$resid <- round(resid(classModel),3)
classData$rstandard <- round(rstandard(classModel),3)
classData$leverage <-round(hatvalues(classModel),3)
classData$cooks <-round(cooks.distance(classModel),3)
View(classData)

#test linearity assumption
classData$intX <- log(x)*x
View(classData)

#rerun model with new interaction variable
linearModel<-glm(y~x+intX, family=binomial(), data=classData)
summary(linearModel)
...
#----Install Packages-----
#check to see if the car package is installed.
...
#********************* Eel Example ********************
#conduct a hierarchical logistic regression
#load data
...
str(eelData)
summary(eelData)
#Re-order the levels of the factyor so that Not Cured and No Treatment are the baseline categories
...
#histogram of our continuous variable
hist(eelData$Duration)
plot(eelData$Duration,eelData$Cured)
plot(eelData$Intervention,eelData$Cured)


#Create the two hierarchical models:
eelModel.0 <- glm(Cured ~ 1, data = eelData, family = binomial()) #Intercept only
eelModel.1 <- glm(Cured ~ Intervention, data = eelData, family = binomial()) #intercept +intervention
eelModel.2 <- glm(Cured ~ Intervention + Duration, data = eelData, family = binomial()) #intercept + Intervention + Duration

#start by Examining only Model.1
summary(eelModel.0)
summary(eelModel.1)
...
anova(eelModel.1,test="Chisq")

#could also do:
anova(eelModel.0, eelModel.1,test="Chisq")



#get R2

1-logLik(eelModel.1)/logLik(eelModel.0)
...
#can do it at once, put them together and round the output

round(exp(cbind(oddsRatio = coef(eelModel.1),confint(eelModel.1))),3)
...
eelData$intDuration<-log(eelData$Duration)*eelData$Duration

linearityModel <- glm(Cured ~ Intervention + Duration+intDuration, data = eelData, family = binomial()) #intercept + Intervention + Duration

#start by Examining only Model.1
summary(eelModel.2)
summary(linearityModel)
...
anova(eelModel.1, eelModel.2, test="Chisq")
